...
function BookShelf() {
    ...
    useEffect(() => {
      const fetchUserInfo = async () => {
        try {
          ...
          const response2 = await axios.get(`https://j10c106.p.ssafy.io:8082/users/${response.data.memberId}/mybookshelf/${detail}`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          });
    ...
    const handleDelete = async () => {
      try {
        const token = localStorage.getItem('jwtToken');
        await axios.delete(`https://j10c106.p.ssafy.io:8082/users/${memberIdRef.current}/mybookshelf/${detail}`, { // memberIdRef.current를 사용
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
